      ...
      <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap', alignItems: 'end' }}>
        <FormControl sx={{ minWidth: 200 }}>
          ...
          <Select value={filters.type} onChange={(e) => onChange({ type: e.target.value as string })}>
            ...
            {['Authentication', 'Authorization', 'Registration', 'Recovery', 'Multi-Factor Authentication'].map(t => <MenuItem key={t} value={t}>{t === 'Multi-Factor Authentication' ? 'MFA' : t}</MenuItem>)}
          </Select>
        </FormControl>
        <FormControl sx={{ minWidth: 200 }}>
          ...
          <Select value={filters.status} onChange={(e) => onChange({ status: e.target.value as string })}>
            ...
            {['active', 'inactive', 'draft'].map(s => <MenuItem key={s} value={s}>{s.charAt(0).toUpperCase() + s.slice(1)}</MenuItem>)}
          </Select>
        </FormControl>
...
const ActionMenu = ({ anchor, onClose, onView, onEdit }: { anchor: HTMLElement | null; onClose: () => void; onView: () => void; onEdit: () => void }) => (
  <Menu anchorEl={anchor} open={Boolean(anchor)} onClose={onClose}>
    ...
    <MenuItem onClick={onClose} sx={{ color: 'error.main' }}>Delete</MenuItem>
  </Menu>
)
...
export default function Components(): JSX.Element {
  ...
  const [filters, setFilters] = useState<Filters>({ type: 'all', status: 'all', query: '' })
  const [density, setDensity] = useState<ListingTableDensity>('standard')
  const [page, setPage] = useState(0)
  const [rows, setRows] = useState(5)
  const [menu, setMenu] = useState<{ el: HTMLElement | null; id: string | null }>({ el: null, id: null })
  ...
  const list = useMemo(() => {
    ...
    return mockComponents.filter(c =>
      ...
      (!q || [c.name, c.type, c.category, c.description].some(s => s.toLowerCase().includes(q)))
    )
  }, [filters])
    ...
    <PageContent>
      <PageTitle>
        ...
        <PageTitle.Actions>
          ...
          <Button variant="contained" startIcon={<Plus size={18} />} onClick={() => navigate(`/projects/${id}/components/new`)}>New Component</Button>
        </PageTitle.Actions>
      </PageTitle>
      ...
      <ListingTable.Provider searchValue={filters.query} onSearchChange={q => setFilters(p => ({ ...p, query: q }))} density={density} onDensityChange={setDensity}>
        <ListingTable.Container disablePaper>
            ...
            <ListingTable.Head>
              <ListingTable.Row>
                {['name', 'type', 'category', 'status', 'author', 'lastModified'].map(f => (
                  <ListingTable.Cell key={f}><ListingTable.SortLabel field={f}>{f.charAt(0).toUpperCase() + f.slice(1)}</ListingTable.SortLabel></ListingTable.Cell>
                ))}
                ...
                <ListingTable.Row>
                  <ListingTable.Cell colSpan={7}>
                    <ListingTable.EmptyState
                      ...
                      action={!filters.query && filters.type === 'all' ? <Button variant="contained" startIcon={<Plus size={16} />} onClick={() => navigate(`/projects/${id}/components/new`)}>Create Component</Button> : undefined}
                    />
                  </ListingTable.Cell>
                </ListingTable.Row>
              ) : (
                    ...
                    <ListingTable.Row key={c.id} variant="card" hover clickable onClick={() => navigate(`/projects/${id}/components/${c.id}`)}>
                      ...
                      <ListingTable.Cell align="right">
                        <ListingTable.RowActions visibility="hover">
                          <IconButton size="small" onClick={(e) => { e.stopPropagation(); setMenu({ el: e.currentTarget, id: c.id }) }}><MoreVertical size={18} /></IconButton>
          ...
          <TablePagination component="div" count={list.length} rowsPerPage={rows} page={page} onPageChange={(_, p) => setPage(p)} onRowsPerPageChange={(e) => { setRows(parseInt(e.target.value, 10)); setPage(0) }} />
      ...
      <ActionMenu
        ...
        onView={() => { navigate(`/projects/${id}/components/${menu.id}`); setMenu({ el: null, id: null }) }}
        onEdit={() => { navigate(`/projects/${id}/components/${menu.id}/edit`); setMenu({ el: null, id: null }) }}
      />
    </PageContent>
  )
}
