...
const Section = ({ title, children, variant = 'outlined', titleColor }: { title: string, children: ReactNode, variant?: any, titleColor?: string }) => (
  <Card variant={variant} sx={{ borderColor: titleColor ? `${titleColor}.main` : undefined }}>
    <CardContent sx={{ p: 3 }}>
      <Typography variant="h5" gutterBottom sx={{ mb: 2, color: titleColor ? `${titleColor}.main` : undefined }}>{title}</Typography>
      <Divider sx={{ mb: 3 }} />
...
const Toggle = ({ check, onChange, label, sub }: { check: boolean, onChange: (v: boolean) => void, label: string, sub: string }) => (
  <Box>
    ...
    <Typography variant="body2" color="text.secondary" sx={{ ml: 5, mt: -1 }}>{sub}</Typography>
  </Box>
)
...
export default function SettingsPage(): JSX.Element {
  ...
  const [data, setData] = useState<SettingsState>({
    general: { orgName: 'Acme Corp', displayName: 'ACME', description: 'Tech solutions', website: 'https://acme.com', industry: 'technology', size: '50-200' },
    appearance: { theme: 'light', language: 'en', timezone: 'utc', dateFormat: 'MM/DD/YYYY' },
    notifications: { email: true, slack: false, security: true, billing: true, weekly: false, updates: true },
    security: { mfa: true, timeout: '30', ipWhitelist: false, domains: '@acme.com' },
  })

  const update = <K extends keyof SettingsState>(section: K, update: Partial<SettingsState[K]>) => {
    setData(prev => ({ ...prev, [section]: { ...prev[section], ...update } }))
    setDirty(true)
  }

  const tabs = [
    {
      icon: <Building2 size={18} />, label: 'General', render: () => (
        <Section title="Organization Information">
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            ...
            <Box>
              ...
              <Typography variant="caption" display="block" color="text.secondary">Square, 200x200px</Typography>
            </Box>
          </Box>
          ...
          <Grid container spacing={2}>
            <Grid size={{ xs: 12, md: 6 }}>
              <Option label="Industry" val={data.general.industry} onChange={v => update('general', { industry: v })}
                opts={[{ v: 'technology', l: 'Technology' }, { v: 'finance', l: 'Finance' }, { v: 'healthcare', l: 'Healthcare' }]} />
            </Grid>
            <Grid size={{ xs: 12, md: 6 }}>
              <Option label="Company Size" val={data.general.size} onChange={v => update('general', { size: v })}
                opts={[{ v: '1-10', l: '1-10' }, { v: '50-200', l: '50-200' }, { v: '500+', l: '500+' }]} />
            </Grid>
          </Grid>
        </Section>
      )
    ...
    {
      icon: <Palette size={18} />, label: 'Appearance', render: () => (
        <Section title="Appearance">
          <FormControl><FormLabel>Theme</FormLabel><ThemeSwitcher /></FormControl>
          <Option label="Language" val={data.appearance.language} onChange={v => update('appearance', { language: v as Language })}
            opts={[{ v: 'en', l: 'English' }, { v: 'es', l: 'Spanish' }, { v: 'fr', l: 'French' }]} />
          <Option label="Timezone" val={data.appearance.timezone} onChange={v => update('appearance', { timezone: v as Timezone })}
            opts={[{ v: 'utc', l: 'UTC' }, { v: 'est', l: 'EST' }, { v: 'pst', l: 'PST' }]} />
          <Option label="Date Format" val={data.appearance.dateFormat} onChange={v => update('appearance', { dateFormat: v })}
            opts={[{ v: 'MM/DD/YYYY', l: 'MM/DD/YYYY' }, { v: 'DD/MM/YYYY', l: 'DD/MM/YYYY' }, { v: 'YYYY-MM-DD', l: 'YYYY-MM-DD' }]} />
        </Section>
      )
    },
    {
      icon: <Bell size={18} />, label: 'Notifications', render: () => (
    ...
    {
      icon: <Shield size={18} />, label: 'Security', render: () => (
        <Section title="Security">
          ...
          <Alert severity="info"><Typography variant="body2">Strong security settings help protect your organization.</Typography></Alert>
        </Section>
      )
    },
    {
      icon: <Users size={18} />, label: 'Members', render: () => (
        <Section title="Team Members">
          <Box display="flex" justifyContent="space-between" mb={2}>
            <Typography variant="h5">Team</Typography>
            <Button variant="contained" size="small" startIcon={<Users size={18} />}>Invite</Button>
          </Box>
          <Divider sx={{ mb: 3 }} />
          <List>
            {[{ n: 'John Doe', e: 'john@acme.com', r: 'Owner', c: 'primary', a: 'JD' }, { n: 'Jane Smith', e: 'jane@acme.com', r: 'Admin', c: 'success', a: 'JS' }]
              .map((m, i) => (
                <ListItem key={i} sx={{ border: 1, borderColor: 'divider', borderRadius: 1, mb: 2 }}>
                  <Avatar sx={{ mr: 2 }}>{m.a}</Avatar>
                  <ListItemText primary={m.n} secondary={<Box display="flex" gap={1} mt={0.5}><Typography variant="body2">{m.e}</Typography><Chip label={m.r} size="small" color={m.c as any} /></Box>} />
                  <Button size="small" variant="outlined" color="error" disabled={m.r === 'Owner'}>Remove</Button>
                </ListItem>
              ))}
          </List>
        </Section>
    ...
    {
      icon: <Key size={18} />, label: 'API Keys', render: () => (
        <Section title="API Keys">
          <Box display="flex" justifyContent="space-between" mb={2}><Typography variant="h5">Keys</Typography><Button variant="contained" size="small">Generate</Button></Box>
          <Divider sx={{ mb: 3 }} />
          <List>
            {[{ t: 'Production', k: 'sk_prod_•••' }, { t: 'Development', k: 'sk_dev_•••' }].map((k, i) => (
              <ListItem key={i} sx={{ border: 1, borderColor: 'divider', borderRadius: 1, mb: 2 }}>
                <ListItemText primary={`${k.t} Key`} secondary={<Box display="flex" gap={1} mt={1}><Typography variant="body2" fontFamily="monospace">{k.k}</Typography><Chip label="Active" size="small" color="success" /></Box>} />
    ...
    {
      icon: <CreditCard size={18} />, label: 'Billing', render: () => (
        <Section title="Billing">
          ...
          <Box p={2} border={1} borderColor="primary.main" borderRadius={1} bgcolor="action.hover" mb={3}>
            <Box display="flex" justifyContent="space-between"><Box><Typography variant="h6">Enterprise</Typography></Box><Typography variant="h5" color="primary">$99/mo</Typography></Box>
          </Box>
          <Button variant="outlined" color="warning">Change Plan</Button>
        </Section>
      )
    },
    {
      icon: <Trash2 size={18} />, label: 'Danger', render: () => (
        <Section title="Danger Zone" variant="outlined" titleColor="error">
          <Alert severity="error" sx={{ mb: 2 }}>Permanent actions.</Alert>
          <Button variant="contained" color="error" startIcon={<Trash2 size={18} />}>Delete Organization</Button>
        </Section>
      )
    },
  ]
  ...
  return (
    <PageContent>
      <Box display="flex" alignItems="center" gap={2} mb={4}>
        ...
        {dirty && <Button variant="contained" startIcon={<Save size={18} />} onClick={() => setDirty(false)}>Save Changes</Button>}
      </Box>

      {dirty && <Alert severity="warning" sx={{ mb: 3 }}>Unsaved changes.</Alert>}
