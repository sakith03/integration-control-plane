...
function avgNonZeroTimeSeries(ts: Record<string, number>): number {
  let s = 0, c = 0;
  for (const v of Object.values(ts)) { if (v > 0) { s += v; c++; } }
  return c > 0 ? s / c : 0;
}
...
function deriveApis(metrics: MetricEntry[]): ApiSummary[] {
  ...
  return Object.entries(apiMap).map(([key, { successful, failed }]) => {
...
function aggregate(metrics: MetricEntry[]) {
  ...
  for (const m of metrics) {
    ...
    for (const [ts, val] of Object.entries(m.requests_total.timeSeriesData)) {
      ...
      if (isFailed) { requestsByTime[ts].failed += val; errorCount += val; } else { requestsByTime[ts].successful += val; }
      totalRequests += val;
    }
  ...
  for (let i = latencyData.length - 1; i >= 0; i--) { if (latencyData[i].p95 > 0) { latestP95 = latencyData[i].p95; break; } }
  ...
  return (
    <Card variant="outlined" sx={{ height: '100%' }}>
      <CardContent>
        ...
        <Typography variant="h4" sx={{ fontWeight: 700, mt: 1, color }}>{value}</Typography>
      </CardContent>
    </Card>
  );
}
...
export default function Metrics(scope: ProjectScope | ComponentScope): JSX.Element {
  ...
  const integrations = useMemo(() => {
    ...
    for (const m of serviceMetrics) { const i = m.tags.integration; if (i) set.add(i); }
    return [...set].sort();
  }, [serviceMetrics]);
  ...
  if (!isComponent && allComponents.length === 0) {
    return <PageContent><EmptyListing icon={<BarChart3 size={48} />} title="No components" description="Add a component to view metrics." /></PageContent>;
  }
  if (environments.length === 0) {
    return <PageContent><EmptyListing icon={<BarChart3 size={48} />} title="No environments" description="Configure an environment to view metrics." /></PageContent>;
  }
    ...
    <PageContent>
      <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 2 }}>
        ...
        <Tooltip title="Refresh">
          <IconButton size="small" onClick={() => refetch()} disabled={!metricsRequest}><RefreshCw size={18} /></IconButton>
        </Tooltip>
      </Stack>
      ...
      <Stack direction="row" gap={2} sx={{ mb: 3 }} flexWrap="wrap" alignItems="center">
        ...
        <Select value={resolution} onChange={(e) => setResolution(e.target.value as string)} size="small" sx={{ minWidth: 140 }} aria-label="Resolution">
          {Object.keys(RESOLUTIONS).map((k) => <MenuItem key={k} value={k}>Resolution: {k}</MenuItem>)}
        </Select>
        {!isComponent && integrations.length > 0 && (
          <Select value={integrationFilter} onChange={(e) => setIntegrationFilter(e.target.value as string)} size="small" sx={{ minWidth: 160 }} aria-label="Integration">
            ...
            {integrations.map((i) => <MenuItem key={i} value={i}>{i}</MenuItem>)}
          </Select>
        )}
      </Stack>
      ...
      {isLoading ? (
        ...
        <Stack alignItems="center" gap={2} sx={{ py: 6 }}>
          <Typography color="error" textAlign="center">Failed to fetch metrics: {(error as Error).message ?? 'Service unavailable'}</Typography>
          <Button variant="contained" startIcon={<RefreshCw size={16} />} onClick={() => refetch()}>Retry</Button>
        </Stack>
        ...
        <>
          ...
          <Grid container spacing={2} sx={{ mb: 3 }}>
            <Grid size={{ xs: 12, sm: 6, md: 3 }}><StatCard title="Total Requests" value={totalRequests.toLocaleString()} /></Grid>
            <Grid size={{ xs: 12, sm: 6, md: 3 }}><StatCard title="Error Count" value={errorCount.toLocaleString()} color="error.main" /></Grid>
            <Grid size={{ xs: 12, sm: 6, md: 3 }}><StatCard title="Error Percentage" value={`${errorPercentage.toFixed(2)}%`} color="error.main" /></Grid>
            <Grid size={{ xs: 12, sm: 6, md: 3 }}><StatCard title="95th Percentile (Latest)" value={`${latestP95.toFixed(2)} ms`} /></Grid>
          </Grid>
          ...
          <Grid container spacing={2} sx={{ mb: 3 }}>
            <Grid size={{ xs: 12, md: 6 }}>
              <Card variant="outlined">
                <CardContent>
                  <Typography variant="h6" sx={{ mb: 1 }}>Requests Per Minute</Typography>
                  <LineChart data={requestsChartData} xAxisDataKey="label" height={350} legend={{ show: true, align: 'center', verticalAlign: 'bottom' }} grid={{ show: true, strokeDasharray: '3 3' }} lines={[
            ...
            <Grid size={{ xs: 12, md: 6 }}>
              <Card variant="outlined">
                <CardContent>
                  <Typography variant="h6" sx={{ mb: 1 }}>Request Latency</Typography>
                  <LineChart data={latencyChartData} xAxisDataKey="label" height={350} legend={{ show: true, align: 'center', verticalAlign: 'bottom' }} grid={{ show: true, strokeDasharray: '3 3' }} lines={[
          ...
          {!isComponent && top5.length > 0 && (
            <>
              <Card variant="outlined" sx={{ mb: 3 }}>
                <CardContent>
                  <Typography variant="h6" sx={{ mb: 2 }}>Top 5 APIs (by Request Count)</Typography>
                  <TableContainer>
                    <Table size="small">
                      <TableHead>
                        <TableRow>
              ...
              <Typography variant="h6" sx={{ mb: 1 }}>Statistics of APIs</Typography>
              <Select
                multiple
              ...
              <Grid container spacing={2}>
                <Grid size={{ xs: 12, md: 6 }}>
                  <Card variant="outlined">
                    <CardContent>
                      <Typography variant="h6" sx={{ mb: 1 }}>Requests Per Minute</Typography>
                      <LineChart data={apiReqData} xAxisDataKey="label" height={350} legend={{ show: true, align: 'center', verticalAlign: 'bottom' }} grid={{ show: true, strokeDasharray: '3 3' }} lines={apiLineKeys.map((k, i) => ({ dataKey: k, name: k, stroke: COLORS[i % COLORS.length], ...LINE_OPTS }))} />
                    </CardContent>
                  </Card>
                </Grid>
                <Grid size={{ xs: 12, md: 6 }}>
                  <Card variant="outlined">
                    <CardContent>
                      <Typography variant="h6" sx={{ mb: 1 }}>Average Request Latency</Typography>
                      <LineChart data={apiLatData} xAxisDataKey="label" height={350} legend={{ show: true, align: 'center', verticalAlign: 'bottom' }} grid={{ show: true, strokeDasharray: '3 3' }} lines={apiLineKeys.map((k, i) => ({ dataKey: k, name: k, stroke: COLORS[i % COLORS.length], ...LINE_OPTS }))} />
                    </CardContent>
                  </Card>
                </Grid>
              </Grid>
