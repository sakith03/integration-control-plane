...
service /auth on httpListener {
    ...
    // ============================================================================
    // MI Runtime User Management Endpoints
    // ============================================================================

    // GET /auth/api/components/{componentId}/runtimes/{runtimeId}/mi-users
    @http:ResourceConfig {
        auth: [
            {
                jwtValidatorConfig: {
    ...
    isolated resource function get api/components/[string componentId]/runtimes/[string runtimeId]/mi\-users(http:Request req)
            returns http:Ok|http:Unauthorized|http:Forbidden|http:NotFound|http:InternalServerError|error {

        log:printInfo("Fetching MI users", componentId = componentId, runtimeId = runtimeId);
        ...
        types:Component|error component = storage:getComponentById(componentId);
        ...
        if hasPermission is error {
            log:printError("Error checking permissions", hasPermission);
            return utils:createInternalServerError("Error checking permissions");
        }
        if !hasPermission {
            return <http:Forbidden>{body: {message: "Insufficient permissions to view MI users"}};
        }

        types:Runtime[] runtimes = check storage:getRuntimes((), (), (), component.projectId, componentId);
        types:Runtime? targetRuntime = ();
        ...
        if targetRuntime is () {
            return <http:NotFound>{body: {message: "Runtime not found"}};
        }

        string baseUrl = check storage:buildManagementBaseUrl(targetRuntime.managementHostname, targetRuntime.managementPort);
        ...
        if mgmtClient is error {
            ...
            return utils:createInternalServerError("Failed to create management API client");
        }

        string|error hmacTokenResult = storage:issueRuntimeHmacToken(runtimeId);
        if hmacTokenResult is error {
            log:printError("Failed to issue HMAC token for runtime", hmacTokenResult, runtimeId = runtimeId);
            return utils:createInternalServerError("Failed to issue authentication token for runtime");
        }
        string bearerToken = hmacTokenResult;
        string listUrl = string `${baseUrl}/management/users`;
        log:printInfo("MI user management: GET /users request",
                runtimeId = runtimeId,
                url = listUrl);
        ...
        if listResponse is error {
            log:printError("MI user management: GET /users failed", listResponse,
                    runtimeId = runtimeId,
                    url = listUrl);
        ...
        if listResponse.statusCode != http:STATUS_OK {
            log:printError("MI user management: GET /users returned non-OK status",
                    ...
                    url = listUrl,
                    statusCode = listResponse.statusCode);
            return utils:createInternalServerError(string `MI management API returned status ${listResponse.statusCode}`);
        }

        json|error listBody = listResponse.getJsonPayload();
        if listBody is error {
            log:printError("MI user management: GET /users failed to parse response", listBody,
                    runtimeId = runtimeId,
                    url = listUrl);
            return utils:createInternalServerError("Invalid response from MI management API");
        }
        ...
        foreach json u in userList {
            ...
            if detailResponse is http:Response {
                ...
                log:printError("MI user management: GET /users/{userId} failed", detailResponse,
                        runtimeId = runtimeId,
                        url = detailUrl);
            }
        ...
        return <http:Ok>{body: {users: enrichedUsers}};
    }

    // POST /auth/api/components/{componentId}/runtimes/{runtimeId}/mi-users
    @http:ResourceConfig {
        auth: [
            {
                jwtValidatorConfig: {
    ...
    isolated resource function post api/components/[string componentId]/runtimes/[string runtimeId]/mi\-users(@http:Payload json payload, http:Request req)
            returns http:Ok|http:BadRequest|http:Unauthorized|http:Forbidden|http:NotFound|http:InternalServerError|error {

        log:printInfo("Creating MI user", componentId = componentId, runtimeId = runtimeId);
        ...
        if !hasPermission {
            return <http:Forbidden>{body: {message: "Insufficient permissions to create MI users"}};
        }

        json|error userId = payload.userId;
        json|error password = payload.password;
        ...
        boolean isAdmin = false;
        json|error isAdminField = payload.isAdmin;
        if isAdminField is boolean {
            isAdmin = isAdminField;
        }
        ...
        string createUrl = string `${baseUrl}/management/users`;
        log:printInfo("MI user management: POST /users request",
                runtimeId = runtimeId,
                url = createUrl,
                userId = userId,
                isAdmin = isAdmin);
        ...
        if createResponse is error {
            log:printError("MI user management: POST /users failed", createResponse,
                    runtimeId = runtimeId,
                    url = createUrl,
                    userId = userId);
        ...
        if createResponse.statusCode != http:STATUS_OK {
            log:printError("MI user management: POST /users returned non-OK status",
                    runtimeId = runtimeId,
                    url = createUrl,
                    statusCode = createResponse.statusCode);
            return utils:createInternalServerError(string `MI management API returned status ${createResponse.statusCode}`);
        }

        json|error responseBody = createResponse.getJsonPayload();
        if responseBody is error {
            return <http:Ok>{body: {userId, status: "Added"}};
        }
        log:printInfo("MI user created successfully", userId = userId, runtimeId = runtimeId);
        return <http:Ok>{body: responseBody};
    }

    // DELETE /auth/api/components/{componentId}/runtimes/{runtimeId}/mi-users/{username}
    @http:ResourceConfig {
        auth: [
            {
                jwtValidatorConfig: {
    ...
    isolated resource function delete api/components/[string componentId]/runtimes/[string runtimeId]/mi\-users/[string username](http:Request req)
            returns http:Ok|http:Unauthorized|http:Forbidden|http:NotFound|http:InternalServerError|error {

        log:printInfo("Deleting MI user", componentId = componentId, runtimeId = runtimeId, username = username);
        ...
        if !hasPermission {
            return <http:Forbidden>{body: {message: "Insufficient permissions to delete MI users"}};
        }
        ...
        string bearerToken = hmacTokenResult;
        string deleteUrl = string `${baseUrl}/management/users/${username}`;
        log:printInfo("MI user management: DELETE /users/{userId} request",
                runtimeId = runtimeId,
                url = deleteUrl,
                username = username);
        ...
        if deleteResponse is error {
            log:printError("MI user management: DELETE /users/{userId} failed", deleteResponse,
                    runtimeId = runtimeId,
                    url = deleteUrl,
                    username = username);
        ...
        if deleteResponse.statusCode != http:STATUS_OK {
            log:printError("MI user management: DELETE /users/{userId} returned non-OK status",
                    runtimeId = runtimeId,
                    url = deleteUrl,
                    statusCode = deleteResponse.statusCode);
            return utils:createInternalServerError(string `MI management API returned status ${deleteResponse.statusCode}`);
        }

        json|error responseBody = deleteResponse.getJsonPayload();
        if responseBody is error {
            return <http:Ok>{body: {userId: username, status: "Deleted"}};
        }
        log:printInfo("MI user deleted successfully", username = username, runtimeId = runtimeId);
        return <http:Ok>{body: responseBody};
    }
}
